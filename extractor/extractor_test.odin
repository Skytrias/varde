...
lower_recognizes_literal_conversion_and_qualified_type_syntax :: proc(t: ^testing.T) {
	...
	testing.expect(t, result.document.types[result.document.entities[4].type].name == "u32be")
	testing.expect(t, result.document.types[result.document.entities[5].type].name == "u32be")
	testing.expect(t, result.document.types[result.document.entities[6].type].name == "untyped integer")
	testing.expect(t, result.document.types[result.document.entities[7].type].name == "untyped integer")
	testing.expect(t, result.document.types[result.document.entities[8].type].name == "untyped integer")
	testing.expect(t, result.document.types[result.document.entities[9].type].name == "untyped integer")
	testing.expect(t, result.document.types[result.document.entities[10].type].name == "untyped float")
	testing.expect(t, result.document.types[result.document.entities[11].type].name == "remote.Value")
	config_count := document_entity_by_name(&result.document, "config_count")
	combined_flags := document_entity_by_name(&result.document, "combined_flags")
	first_flag := document_entity_by_name(&result.document, "first_flag")
	testing.expect(t, config_count != nil && result.document.types[config_count.type].name == "untyped integer", "#config defaults and min/max should infer a compatible untyped integer type")
	testing.expect(t, combined_flags != nil && first_flag != nil && combined_flags.type == first_flag.type, "same-type local bit-set sums should retain their operand type")
}
