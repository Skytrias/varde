...
site_head :: proc(builder: ^strings.Builder, page_title, project_title, relative_assets, site_root: string, config: Config, extensions: Site_Extensions) {
	...
	strings.write_string(builder, "\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><meta name=\"color-scheme\" content=\"light dark\"><title>")
...
:root[data-theme="dark"] { --bg:#101511; --surface:#18211a; --surface-raised:#1d2820; --text:#e7eee8; --muted:#a6b7a9; --line:#344338; --accent:#77d9a4; --accent-ink:#092016; --code:#0c130e; --focus:#75c7f5; }
...
SITE_JS :: `
(() => {
  const root=document.documentElement,themeKey="varde-theme",toggle=document.querySelector("#theme-toggle");
  let saved="";try{saved=localStorage.getItem(themeKey)||""}catch(_){}if(saved==="light"||saved==="dark")root.dataset.theme=saved;
  const themeIsDark=()=>root.dataset.theme?root.dataset.theme==="dark":matchMedia("(prefers-color-scheme: dark)").matches;
  const updateThemeButton=()=>{if(!toggle)return;const dark=themeIsDark();toggle.textContent=dark?"Light":"Dark";toggle.setAttribute("aria-pressed",String(dark));};
  updateThemeButton();toggle?.addEventListener("click",()=>{const next=themeIsDark()?"light":"dark";root.dataset.theme=next;try{localStorage.setItem(themeKey,next)}catch(_){}updateThemeButton();});
...
test_build_emits_directly_openable_site :: proc(t: ^testing.T) {
	...
	testing.expect(t, index_read_err == nil && strings.contains(string(index_data), "data-default-theme=\"dark\"") && strings.contains(string(index_data), "data-default-tab-width=\"2\""), "project presentation defaults should reach every generated page")
