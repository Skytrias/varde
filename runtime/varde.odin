...
site_head :: proc(builder: ^strings.Builder, page_title, project_title, relative_assets, site_root: string, config: Config, extensions: Site_Extensions) {
	...
	strings.write_string(builder, "\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><meta name=\"color-scheme\" content=\"light dark\"><script>")
...
SITE_THEME_BOOTSTRAP_JS :: `(()=>{const r=document.documentElement,d=r.dataset,k="varde-settings",validTheme=v=>v==="light"||v==="dark"||v==="system",validMotion=v=>v==="system"||v==="full"||v==="reduced",validTab=v=>v==="2"||v==="4"||v==="8";let raw="";try{raw=localStorage.getItem(k)||""}catch(_){}if(!raw)try{const bag=window.name?JSON.parse(window.name):{};raw=typeof bag.__varde_settings_v1__==="string"?bag.__varde_settings_v1__:""}catch(_){}let saved={};try{saved=JSON.parse(raw||"{}")||{}}catch(_){}const theme=validTheme(saved.theme)?saved.theme:d.defaultTheme||"system",motion=validMotion(saved.motion)?saved.motion:d.defaultMotion||"system",tab=validTab(String(saved.tabWidth))?String(saved.tabWidth):d.defaultTabWidth||"4";if(theme==="light"||theme==="dark")d.theme=theme;else delete d.theme;d.motion=motion;r.style.setProperty("--code-tab-width",tab)})();`
